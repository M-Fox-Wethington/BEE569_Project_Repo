...
cat("
  ...
  for (i in 1:N.patches) #move through all ice floes
    ...
    logit(lambda[i]) <- b0.lam + betaArea.lam * Area.lam[i] #abundance models as a function of floe size
    
    Occupancy[i] ~ dbern(p[i]) #occupancy of ith ice floe (Occupancy = 1 if occupied) and probability depends on floe area
    logit(p[i]) <- b0.occ + betaArea.occ * Area.occ[i]  #probability of occupancy 
    }
  ...
  #Occupancy priors
  # p ~ dbeta (1,1)   #p is a probability so beta prior is fine
  # b0.occ <- logit(p0)   #intercept is the probability of occupancy on the logit scale
  # p0 ~ dbeta(1,1)
  b0.occ ~ dunif(0,5)
...
Dat <- list(
  N.patches = nrow(seals.subset.3000),
  Count = as.numeric(seals.subset.3000$Count),
  Occupancy = ifelse(seals.subset.3000$presence > 0, 1, NA),
  Area.lam = as.numeric(seals.subset.3000$area),
  Area.occ = as.numeric(seals.subset.3000$area))


# # # Function for initial parameter values
inits = list(list(b0.lam = runif(1, 0, 5),  betaArea.lam = runif(1, 0, 5), p0 = rbeta(1, 0, 1), b0.occ = runif(1, 0, 5),  betaArea.occ = runif(1, 0, 5)),
             list(b0.lam = runif(1, 0, 5),  betaArea.lam = runif(1, 0, 5), p0 = rbeta(1, 0, 1), b0.occ = runif(1, 0, 5),  betaArea.occ = runif(1, 0, 5)),
             list(b0.lam = runif(1, 0, 5),  betaArea.lam = runif(1, 0, 5), p0 = rbeta(1, 0, 1), b0.occ = runif(1, 0, 5),  betaArea.occ = runif(1, 0, 5)))

# 
# 
# inits = list(list(alpha.lam = rnorm(1, 0, 0.01),  beta1.lam = rnorm(1, 0, 0.01), alpha.occ = rnorm(1, 0, 0.01), beta1.occ = rnorm(1, 0, 0.01)),
#              list(alpha.lam = rnorm(1, 0, 0.01),  beta1.lam = rnorm(1, 0, 0.01), alpha.occ = rnorm(1, 0, 0.01), beta1.occ = rnorm(1, 0, 0.01)),
#              list(alpha.lam = rnorm(1, 0, 0.01),  beta1.lam = rnorm(1, 0, 0.01), alpha.occ = rnorm(1, 0, 0.01), beta1.occ = rnorm(1, 0, 0.01)))


#Parameters we want to track
ParsStage <- c("b0.lam","betaArea.lam", 
               "b0.occ", "betaArea.occ", "lambda", "p")


#Variables for MCMC
ni <- 74500  # number of draws from the posterior
nt <- 2    #thinning rate
nb <- 4500  # number to discard for burn-in
...
#save(Mod2_duo_deep , file="Mod2_RealData_duo_long.RData")
