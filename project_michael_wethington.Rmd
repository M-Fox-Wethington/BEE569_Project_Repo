...
cat("
  ...
  for (i in 1:N.patches) #move through all ice floes
    ...
    logit(p[i]) <- alpha.occ + beta1.occ * Area.occ[i]  #probability of occupancy 
    }
...
# inits = list(list(alpha.lam = runif(1, -5, 5),  beta1.lam = runif(1, -5, 5), p0 = rbeta(1, 0, 1) ),
#              list(alpha.lam = runif(1, 0, 10),  beta1.lam = runif(1, 0, 10), p0 = rbeta(1, 0, 1) ),
#              list(alpha.lam = runif(1, -10, 0),  beta1.lam = runif(1, -10, 0), p0 = rbeta(1, 0, 1)))
...
#Variables for MCMC
ni <- 52500  # number of draws from the posterior
nt <- 5   #thinning rate
nb <- 2500  # number to discard for burn-in
...
save(Mod2_duo , file="Mod2_RealData_duo.RData")
...
MCMCtrace(Mod2_duo, ISB = FALSE, pdf = FALSE )
